...
export default function ContactFeedback() {
  ...
  return (
    <section className="py-12 md:py-24 bg-perfume-main relative overflow-hidden">
      ...
      <div className="container-lux relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-24">
          ...
          <div className="lux-glass-card p-5 sm:p-8 md:p-14">
              ...
              {success ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-8">
                  ...
                  <div className="flex flex-col gap-3 group">
                    ...
                    <textarea id="message" value={form.message} onChange={handleChange} rows={3} className="bg-transparent border-b border-foreground/10 py-2 text-sm focus:outline-none focus:border-gold transition-colors resize-none placeholder:text-foreground/20 text-foreground font-light tracking-wide" placeholder="Your message..." required></textarea>
                  </div>
                  {error && <p className="text-red-400 text-sm">{error}</p>}
                  <button type="submit" disabled={isSubmitting} className="mt-2 sm:mt-6 w-full bg-gold text-background py-3 sm:py-4 text-xs font-bold tracking-[0.2em] hover:bg-foreground transition-all duration-300 disabled:opacity-50">

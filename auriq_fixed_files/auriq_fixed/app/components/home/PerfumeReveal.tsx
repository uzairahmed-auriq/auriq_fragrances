...
// 24 particles in a tight mist fan, upward spray direction
const PARTICLES = Array.from({ length: 24 }, (_, i) => ({
  id: i,
  angle: -110 + i * 9.5,          // -110° to +100°, top-facing fan
  distance: 35 + (i % 6) * 18,    // 35px – 125px
  size: 2 + (i % 4),              // 2–5px
  opacity: 0.4 + (i % 5) * 0.12, // 0.4 – 0.88
}));
...
export default function PerfumeReveal() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      current = lerp(current, progress, 0.08);
  ...
  const s1 = easeInOut(Math.min(p / 0.25, 1));
  const s2 = easeInOut(Math.min(Math.max((p - 0.25) / 0.25, 0), 1));
  const s3 = easeInOut(Math.min(Math.max((p - 0.50) / 0.25, 0), 1));
  const s4 = easeInOut(Math.min(Math.max((p - 0.75) / 0.25, 0), 1));
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow:  `0 0 ${pt.size * 2}px rgba(212,175,55,0.5)`,
